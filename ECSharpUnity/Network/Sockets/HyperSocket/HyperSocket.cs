...
namespace ECSharp.Network.Sockets.HyperSocket
{
    ...
    public class HyperSocket : BaseHyperSocket
    {
        ...
        public bool SendTcp(byte[] data)
        {
            if (IsValid)
            {
                if (aes != null && (config.SSLMode == 0 || config.SSLMode == 1))
                    return tcpClient.Send(SessionId, aes.Encrypt(data));
                else
        ...
        public void SendUdp(byte[] data)
        {
            ...
            {
                if (aes != null && (config.SSLMode == 0 || config.SSLMode == 2))
                    udpClient.SendKcp(aes.Encrypt(data));
                else
                    udpClient.SendKcp(data);
            }
        }
        ...
        public void SendTcp(Var value)
        {
            SendTcp(value.GetBytes());
        }
        ...
        /// </summary>
        /// <param name="value"></param>
        public void SendUdp(Var value)
        {
            SendUdp(value.GetBytes());
        }
        ...
        /// <param name="isTcpMode">默认tcp模式，否则udp模式</param>
        public void Send(Var value, bool isTcpMode = true)
        {
            if (isTcpMode)
                SendTcp(value);
            else
