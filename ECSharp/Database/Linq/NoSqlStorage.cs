...
namespace ECSharp.Database.Linq
{
    ...
    public class NoSqlStorage<T, U> : ITimeUpdate where T : IComparable where U : IComparable
    {
        ...
        public NoSqlStorage(IDbHelper dBHelper, string keyName, string valueName, string tableName, int syncPeriod = 1000, string condition = "")
        {
        ...
        public bool TryGetValue(T key, out U? value)
        {
            ...
            {
                var result = dBHelper.CommandSQL($"SELECT TOP 1 [{valueName}] FROM {tableName} WHERE {condition} {keyName}='{key}'");
                if (result.EffectNum > 0 && result.Rows != null)
                {
                    var val = value = (U)result.Rows[0][valueName];
