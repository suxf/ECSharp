...
namespace ES.Network.HyperSocket
{
    ...
    public class HyperSocketServer : BaseHyperSocket
    {
        ...
        /// 远程连接
        /// </summary>
        internal readonly ConcurrentMap<ushort, RemoteHyperSocket?> remoteSockets;
        ...
        : base(true, ip, tcpPort, udpPort, connectMaxNum, config)
        {
            ...
            else if (connectMaxNum > ushort.MaxValue - 1) connectMaxNum = ushort.MaxValue - 1;

            // 赋值最大连接数量 这样做的目的是因为 0 索引位被占用导致的
            ConnectMaxNum = connectMaxNum;

            remoteSockets = new ConcurrentMap<ushort, RemoteHyperSocket?>();
            for (ushort i = 1; i < connectMaxNum; i++)
            {
                remoteSockets.TryAdd(i, null);
            }

            if (config?.UseSSL ?? false) rsa = new RsaCrypto();
        ...
        internal ReadOnlySpan<byte> GenerateVerifyConnection(out ushort sessionId)
        {
            ...
            if (sessionId <= ushort.MinValue) return null;
            // 先加入验证
        ...
        */
        ...
        {
            if (index < 0 || index >= ConnectMaxNum) return;
